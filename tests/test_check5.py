...
#
# Three branches alone is too noisy to fire Check 5 by itself, but three
# branches whose returns also don't depend on the input is almost certainly
# enumeration-as-hack. The global standalone Check 5 threshold stays at 5;
# the combination path only fires when both signals co-occur for the same
# function.
# ---------------------------------------------------------------------------

def test_b4_three_branch_input_independent_enumeration_flags_in_standalone():
    # if/elif/elif (not three top-level guard ifs) so the enumeration analyzer
    # actually counts the branches; three branches alone is below the global
    # min_ifs=5 threshold but co-fires here because returns are
    # input-independent literals.
...
"""
    ...
    assert c5["status"] == "WARNING", (
        f"Expected Check 5 WARNING via B4 combination; got {c5['status']}. "
        f"Findings: {c5['findings']}"
    )
    # The escalation finding should reference the combination explicitly.
    explanations = " ".join(f["explanation"] for f in c5["findings"])
    assert "input_independent_returns" in explanations, (
        f"Expected combination escalation note; got: {explanations}"
    )


def test_b4_three_branch_genuine_dispatch_does_not_flag_check5():
    # Three constant-equality branches but each return USES the input → not
    # input-independent → B4 combination must not fire and Check 5 must stay
    # CLEAN (3 ifs is below the global threshold of 5).
    from ast_guard import scan_standalone
    code = """
def dispatch(x, mode):
    if mode == "double":
